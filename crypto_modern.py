...
def analyze_target(target: Path) -> tuple[str, list[str]]:
    """Analysiert eine Datei oder einen Ordner rekursiv."""

    if target.is_file():
        indicators = find_ransomware_indicators(target)
        return analyze_suspicious_file(target), indicators
...
class CryptoApp(TkinterDnD.Tk):
    ...
    def run_background(self, task, on_success, action: str, target: Path) -> None:
        if self.busy:
    ...
    def choose_folder(self) -> None:
        path = filedialog.askdirectory(title="Ordner zur Prüfung auswählen")
        if path:
            self.set_selected_file(path)
    ...
    def encrypt_selected(self) -> None:
        ...
        if not self.selected_file.is_file():
            messagebox.showwarning(
                "Ordner ausgewählt",
                "Zum Verschlüsseln bitte eine einzelne Datei auswählen.",
            )
            return
        ...
        def task():
            ...
            return backup, source_hash, sha256_file(output_path)
        ...
        self.run_background(task, finished, "Datei verschlüsselt", source)
    ...
    def decrypt_selected(self) -> None:
        ...
        if not self.selected_file.is_file():
            messagebox.showwarning(
                "Ordner ausgewählt",
                "Zum Entschlüsseln bitte eine einzelne Datei auswählen.",
            )
            return
        ...
        def task():
            ...
            return backup, sha256_file(output_path)
        ...
        self.run_background(task, finished, "Datei entschlüsselt", source)
